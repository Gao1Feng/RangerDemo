...
import com.musk.service.Ranger;
import com.musk.utils.PropertyUtil;
...
public class RangerImpl implements Ranger {
    ...
    public String getAllValidPolice() {
        ...
        String url = rangerBaseUrl + "/service/plugins/policies/download/" + hdfsService;
    ...
    public boolean createPolice3() {
        ...
        try {
            ...
            rangerPolicy.setName("cl1_test-1");
            rangerPolicy.setIsEnabled(true);
            rangerPolicy.setIsAuditEnabled(true);
            rangerPolicy.setDescription("Policy for Service: cl1_test");
            List<RangerPolicy.RangerPolicyItem> denyExceptions = new ArrayList<>();
            rangerPolicy.setDenyExceptions(denyExceptions);
            List<RangerPolicy.RangerPolicyItem> allowExceptions =new ArrayList<>();
            ...
            RangerPolicy.RangerPolicyResource pathPolicyResource = new RangerPolicy.RangerPolicyResource();
            pathPolicyResource.setValue("abc");
            pathPolicyResource.setIsExcludes(false);
            pathPolicyResource.setIsRecursive(false);
            resources.put("path",pathPolicyResource);
            rangerPolicy.setResources(resources);

            System.out.println(JSON.toJSONString(rangerPolicy));
            response = webResource.accept(RangerRESTUtils.REST_EXPECTED_MIME_TYPE)
                    .type(RangerRESTUtils.REST_EXPECTED_MIME_TYPE)
                    .post(ClientResponse.class, JSON.toJSONString(rangerPolicy));
            if (response != null && response.getStatus() == 200) {

                RangerPolicy rangerPolicyResponse = response.getEntity(RangerPolicy.class);
                log.info("Create Police is success, the police message is=" + rangerPolicyResponse);
                ret = true;
            } else {
    ...
    public boolean updatePolice3() {
        boolean ret = false;
        String url = rangerBaseUrl + "/service/public/v2/api/policy/"+15;
        log.info("CreatePolice of reqUrl=" + url);
        ClientResponse response = null;
        Client client = null;
        try {
            ...
            response = webResource.accept(RangerRESTUtils.REST_EXPECTED_MIME_TYPE)
                    .type(RangerRESTUtils.REST_EXPECTED_MIME_TYPE)
                    .put(ClientResponse.class, JSON.toJSONString(rangerPolicy));
            if (response != null && response.getStatus() == 200) {
    ...
    public boolean deletePoliceByPoliceName(String policeName) {
        ...
        boolean flag = false;
        String url = rangerBaseUrl + "/service/public/v2/api/policy?servicename=" + hdfsService + "&policyname="
                + policeName;
     ...
     *
     * @param req
     * @param policyType 策略类型，分别为 hive/hbase/hdfs
     * @return
     */
...
//        UpdatePoliceReq hbase = transJsonToObject(rangerImpl.getPolicyByName("testHbase", "hbase"), "hbase");
        UpdatePoliceReq hdfs = transJsonToObject(rangerImpl.getPolicyByName("celue", "hdfs"), "hdfs");
...
//        System.out.println("-------hbase--------");
        ...
        //创建策略
//        System.out.println(rangerImpl.createPolice3());
        //修改策略
        System.out.println(rangerImpl.updatePolice3());
...
//         System.out.println("system out: " + allValidPolice);
        ...
        // // 根据策略名获取
//         String response = rangerImpl.getPolicyByName("celue","hdfs");
//         System.out.println(response);
//
//         UpdatePoliceReq oldPolice = transJsonToObject(response,"hdfs");
//         System.out.println(oldPolice.toString());
        // 删除策略（根据策略名称）
//         boolean flag =
//         rangerImpl.deletePoliceByPoliceName("test333");
//         System.out.println(flag);

        // 删除策略（根据策略ID）
//         boolean flag = rangerImpl.deletePoliceByPoliceId("14");
//         System.out.println(flag);

        // 创建策略
//        CreatePoliceReq createPoliceReq = new CreatePoliceReq();
//        createPoliceReq.setPoliceUser("ranger");
